...
for short_target in ${TARGET[@]}; do
  ...
  cross build --target $target \
              --no-default-features \
              --features 'basic, mimalloc, local-tunnel, aead-cipher-2022, local-dns, local-redir, local-online-config' \
              --release
...
git clone https://github.com/zephyrchien/kaminari --depth=1 kaminari-src
pushd kaminari-src
